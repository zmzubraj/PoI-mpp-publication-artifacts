...
- `E3_V2_DEVELOPMENT_HANDOFF.md`
  - fail-closed Phase-3 handoff for the 120–150-item real-model development set, accountable annotation/taxonomy review, deterministic calibration freeze, and development/confirmatory separation; it is not evidence and does not support C3
...
Fresh-current revalidation on 2026-08-25 is fail-closed: the present repository request manifest and the files currently under `/Users/rainbow/Documents/POI_E3_EXTERNAL` no longer form the same hash chain recorded by that signed-revision receipt. The detached signatures still verify for the declared signer key and the four result artifacts still match the attestation's artifact hashes, but the current request-manifest and authority-record bindings do not match. This drift does not erase or upgrade the retained negative result; it blocks describing the current exchange directory as a freshly verified end-to-end authority/attestation package until the accountable evaluator reissues a consistent current chain or restores the exact signed files.
