| Topic | Category | MPP status | Note |
|---|---|---|---|
| 1B–3B open-weight instruct model | Primary/In scope | In scope | Core prototype serving target. |
| Optional 7B–8B scaling run | Primary/In scope | In scope | Optional extension within the MPP rather than a required result. |
| Objective and grounded evidence tasks | Primary/In scope | In scope | These define the current high-value verification classes. |
...
| Production H100/B200 confidential-computing lane | Deferred/Out of scope | Deferred | Discussed architecturally but not claimed as executed MPP evidence. |
| Production decentralized DA network | Deferred/Out of scope | Deferred | Current work remains below that deployment boundary. |
| New production L1 consensus client | Deferred/Out of scope | Deferred | Not part of the first publication evidence package. |
| Universal open-ended semantic intelligence verification | Deferred/Out of scope | Deferred | Explicitly outside the current operational claim boundary. |
