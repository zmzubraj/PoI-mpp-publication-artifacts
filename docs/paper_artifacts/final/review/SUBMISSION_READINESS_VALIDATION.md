...
**Gap:** Mandatory re-execution or universal proof generation makes common-path verification of AI responses costly. **Question:** Can one committed model execution expose post-commit audit surfaces and mature into bounded protocol authority without overstating unverified semantics or production consensus security? **Method:** SPAI binds task, model, response, trace, evidence, audit, challenge/data-availability, receipt, credit, and next-epoch weight state. **Evidence:** E1 and E2 real-model pilots, E4-E6 and E8 reproducible simulations, and E7 local Foundry measurements. **Limitations:** E1/E2/E4/E8 are inconclusive, E5/E6 are simulation-only, E7 is local-only, and E3 is absent pending external authority. **Implication:** the repository supports a technically coherent, fail-closed MPP artifact architecture, not general semantic verification or production consensus security.

## Executive verdict

- Supported developmental stage: `TECHNICALLY_PROMISING`
- Submission status: `NOT_READY`
- Lowest-stage rationale: a substantial, reproducible narrow MPP exists, but central semantic evidence, independent expert review, venue selection, and the final freeze are absent; strongest-prior-art novelty has not been independently closed
...
| E3 semantic execution | T4/T8/F7 omission records state `WAITING_EXTERNAL_EVALUATOR_AUTHORITY` | `MISSING_EXTERNAL` | real external authority record, authorized execution, raw provenance, and generated T4/T8/F7 |
...
| KQ-05 | Is semantic verification empirically supported? | `FAIL` | E3 T4/T8/F7 are explicit `WAITING_EXTERNAL` omissions | central semantic-performance claims are inadmissible | obtain authority and run the authorized held-out protocol |
...
|---|---|---|---|---|
| STOP-1 | `OPEN` | semantic assurance / C3 | no external evaluator authority or authorized E3 result exists | independently issued authority record plus authorized real execution |
...
| semantic assurance | none | none | no result | external authority absent | idea/protocol only |
...
The paper presents an ambitious architecture, but the central semantic experiment is absent and most other evidence is either a tiny pilot or simulation, leaving insufficient evidence for a broad publication claim.
...
The repository is unusually disciplined, but an unfrozen worktree, missing external records, and incomplete release packaging prevent independent end-to-end reproduction of the submission claim.
...
The evidence-origin rules are strong, but authorship, conflicts, funding, AI-use disclosure, external authority, and reviewer accountability must be completed by real responsible humans.

## Prioritized remediation

| Priority | Action | Owner | Required evidence | Acceptance test | Dependencies | Effort | New data? |
|---|---|---|---|---|---|---|---|
| P0 | obtain E3 evaluator authority | external accountable evaluator | completed authority record, identity basis, exact reviewed hashes, detached signature | schema and signature verification pass | selected evaluator and privacy scope | external | yes |
